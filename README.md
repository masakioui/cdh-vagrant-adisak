...
The idea of repository was inspired by [frederickayala](https://github.com/frederickayala/cdh-vagrant). Most of the configuration is taken from there. 
...
- This works with Mac OS, Linux, and Windows 10.
    -   For Windows 10:
        ...
        - Clone the repository to `d:/cdh-vagrant-adisak/` which is accessible from `/mnt/d/cdh-vagrant-adisak/` in WSL
...
## Installation Guide
- Clone this repository using `git clone https://github.com/masakioui/cdh-vagrant-adisak.git`
    - If you are using Windows the repository must be cloned to a Windows drive path (e.g., `D:/cdh-vagrant-adisak` or `/mnt/d/cdh-vagrant-adisak`
- Go to the repository folder using `cd cdh-vagrant-adisak` 
...
- SSH to the node `vagrant ssh m1` the password is `vagrant`
...
- Run the Cloudera Manager Installer:
    ...
    - When the installation is finished you will see a message saying that you should open your Web browser to `http://m1.cdh.cluster1:7180/`

## Setting up the Cluster
- Open your browser and go to `http://m1.cdh.cluster1:7180/` if it does not work please check your hosts file or wait a couple of minutes for the process to start.
- The default username and password is `admin`
- Click `Continue` and accept the terms and conditions
- Select `Cloudera Express`, click `Continue`, and name your cluster `Cluster 1`
- In `Specify Hosts` section write `m[1-2].cdh.cluster1,w[1-5].cdh.cluster1` and click `Search`. If you are using more than 3 VMs modify accordingly. A list of the three hosts should appear saying `Host was successfully scanned.` Click `Continue`.
...
- In the `Enter Login Credentials` section:
    - In Authentication Method select: `User `
...
- In the `Select services` section you can pick the services that you want.
    ...
    - Note that 7 VMs is very little computing power for such services so choose only what you want to try.
...
- Eventually you will see `Status  Finished`. Click on the `Cloudera Manager` logo or go to `http://m1.cdh.cluster1:7180/`
